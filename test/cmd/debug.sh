...
run_kubectl_debug_warning_tests() {
  ...
  # Command: add a new debug container with netadmin profile
  output_message=$(kubectl debug target -it --image=busybox --attach=false -c debug-container --profile=netadmin "${kube_flags[@]:?}" 2>&1)
  kube::test::if_has_string "${output_message}" 'Warning: Non-root user is specified for the entire target Pod and some capabilities granted by debug profile may not work. Please consider using "--custom" with the custom profile specifying "securityContext.runAsUser: 0".'
